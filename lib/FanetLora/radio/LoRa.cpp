  ...
  switch (radioType){
    ...
    case RADIO_SX1276:
      if (_fskMode){
        ...
        if(_freq < 868.0) {
          lastPacketRSSI = -164 + pGxModule->SPIgetRegValue(SX127X_REG_PKT_RSSI_VALUE); //REG_PKT_RSSI_VALUE
        } else {
          lastPacketRSSI = -157 + pGxModule->SPIgetRegValue(SX127X_REG_PKT_RSSI_VALUE); //REG_PKT_RSSI_VALUE
        }
...
float LoRaClass::getSNR(){
  switch (radioType){
    case RADIO_SX1276:
      ...
      // get SNR value
      return (int8_t)pGxModule->SPIgetRegValue(SX127X_REG_PKT_SNR_VALUE) / 4.0; //REG_PKT_SNR_VALUE
  }
  return 0.0;
}
...
int16_t LoRaClass::switchFSK(float frequency){
  ...
  switch (radioType){
    ...
    case RADIO_SX1276:
      ...
      pGxModule->SPIwriteRegister(SX127X_REGBitrateMSB,0x01); //RegBitrateMsb 100kbps
      pGxModule->SPIwriteRegister(SX127X_REGBitrateLSB,0x40); //RegBitrateLsb
      //set deviation to 50kHz
      pGxModule->SPIwriteRegister(SX127X_REGFdevMSB,0x03); //RegFdevMsb Frequency deviation +/- 50kHz
      pGxModule->SPIwriteRegister(SX127X_REGFdevLSB,0x33); //RegFdevLsb
      //calculate register values
      uint32_t FRF = (frequency * (uint32_t(1) << 19)) / 32.0;
      // write registers
      pGxModule->SPIsetRegValue(SX127X_REG_FRF_MSB, (FRF & 0xFF0000) >> 16); //RegFrMsb
      pGxModule->SPIsetRegValue(SX127X_REG_FRF_MID, (FRF & 0x00FF00) >> 8);  //RegFrMid
      pGxModule->SPIsetRegValue(SX127X_REG_FRF_LSB, FRF & 0x0000FF); //RegFrLsb

      //pGxModule->SPIwriteRegister(0x09,0xFC); //RegPaConfig
      pGxModule->SPIwriteRegister(SX127X_REG_PA_CONFIG,0xFF); //RegPaConfig PA_Boost on, max_power=15, Output Power 17dBm
      pGxModule->SPIwriteRegister(SX127X_REG_PA_RAMP,0x49); //RegPaRamp
      pGxModule->SPIwriteRegister(SX127X_REG_OCP,0x2B); //RegOcp
      pGxModule->SPIwriteRegister(SX127X_REG_LNA,0x23); //RegLna max gain, default LNA current
