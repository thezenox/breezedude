				...
				if ((ppsDiff) > actPPsDiff){
					if (fmac.flarmZone == 1){
						if ((millis() - fmac._ppsMillis) >= (LEGACY_8682_BEGIN - LEGACY_RANGE)){
							...
							#if RX_DEBUG > 5
							log_i("**** %d start FSK 868.2 %d %d\n",millis(),millis() - ppsMillis,fmac._ppsCount);
							#endif
						}
...
//Serial.print("fmac._actMode = "); Serial.println(fmac._actMode);
//Serial.print("fmac._RfMode.bits.FntTx = "); Serial.println(fmac._RfMode.bits.FntTx);
//Serial.print("fmac.eLoraRegion = "); Serial.println(fmac.eLoraRegion);
...
void FanetMac::handleTx()
{
	...
	{
		//Serial.println("still in backoff or chip turned off");
		//return;
	}
	...
	if (myApp->is_broadcast_ready(neighbors.size()))
	{
		...
		if(current_qlen > 0) {printf("### tx current queue: %d\r\n", current_qlen);}
	...
	if (state != ERR_NONE) {
			log_e("startReceive failed, code %d\r\n",state);
	}
	delete[] buffer;
	...
	else if (tx_ret == TX_RX_ONGOING || tx_ret == TX_TX_ONGOING)
	{
		...
		if(tx_ret == TX_RX_ONGOING)
			Serial.println("rx, abort.\r\n");
		else
			Serial.println("tx not done yet, abort.\r\n");
#endif
...
//	//printf("MAC:");Serial.println(uint64ToString(chipmacid)); // six octets

	//for(size_t i = 0; i < UniqueIDsize; i++){
	//	Serial.print(UniqueID[i], HEX); Serial.print(" ");
	//}
 // Serial.print("\n");

	uint8_t myDevId[3];
	myDevId[0] = ManuId;//Manufacturer GetroniX
	myDevId[1] = uint8_t(UniqueID[0]); //last 2 Bytes of MAC
	myDevId[2] = uint8_t(UniqueID[1]);
