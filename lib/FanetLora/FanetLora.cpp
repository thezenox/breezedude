...
bool FanetLora::checkSendComplete(){
  return fmac.txQueueDepleted();
}
...
bool FanetLora::frm2txBuffer(Frame *frm){
  //log_i("payload_length=%i\n",frm->payload_length);
  //log_i("%s\r\n",CreateFNFMSG(frm).c_str());
  if (!fmac._RfMode.bits.FntTx){
    log_i("FanetTx not enabled\r\n");
  ...
  if(fmac.transmit(frm) == 0){
    ...
    //log_i("TX OK\n");
     // Serial1.print("Tx Queue: ");
     // Serial1.println(fmac.getTxFifo());
    return true;
  }else{
		delete frm;
		log_e("TX-buffer full2\r\n");
    return false;
  }
  //log_i("ready");
}
