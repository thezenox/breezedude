...
int FlarmDataPort::writeFlarmData(char *buffer, size_t size,FlarmtrackingData *myData,FlarmtrackingData *movePilotData){
    ...
    */
    snprintf(buffer,size,"$PFLAA,0,%ld,%ld,%ld,2,%s,%ld,0,%.01f,%.01f,%X",(int32_t)round(relNorth),(int32_t)round(relEast),(int32_t)round(relVert),movePilotData->DevId.c_str(),(int32_t)round(movePilotData->heading),currentSpeed,movePilotData->climb,uint8_t(movePilotData->aircraftType));
